...
# fdi <- fdi %>%
#   mutate(eleven = case_when(
#     .$Destination_country %in% aid_11 ~ 1,
#     TRUE ~ 0
#     ))
# fdi$eleven
# 
# fdi_10 <- subset(fdi, Destination_country %in% nonaid_10)
# 
# colnames(fdi)
# 
# round(prop.table(table(fdi$eleven, fdi$Sector), margin = 1), 2)
...
for (country in all_countries) {
  ...
  # Save the data -----------------------------------------------------------
  save(data_respno_FDI,file=paste(country,"_data_respno_FDI.RData", sep = ""))
